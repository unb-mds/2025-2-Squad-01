...
export default function PullRequestsPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        setLoading(true);
        setError(null);
        // Fetch from local public folder during development, fallback to GitHub for production
        const isDevelopment = window.location.hostname === 'localhost';
        const dataUrl = isDevelopment 
          ? '/2025-2-Squad-01/temporal_events.json'
  ...
  const histogramData = selectedRepoData
    ? Object.entries(
        selectedRepoData.activities.reduce((acc, activity) => {
  ...
  const pieChartData = selectedRepoData
    ? (() => {
        const userCounts = selectedRepoData.activities.reduce((acc, activity) => {
          const user = activity.user.displayName || activity.user.login;
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div className="border rounded-lg" style={{ backgroundColor: '#222222', borderColor: '#333333' }}>
                  <p className="text-left p-6 font-bold text-white mb-4">Pull Requests timeline</p>
